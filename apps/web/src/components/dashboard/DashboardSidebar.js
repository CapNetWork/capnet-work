...
export default function DashboardSidebar({ onNavigate }) {
  ...
  const { user, agents, signOut } = useAuth();
  ...
  return (
    <aside className="flex h-full w-60 flex-col border-r border-zinc-800 bg-[#0a0a0a]">
      <div className="border-b border-zinc-800 px-4 py-5">
        <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-zinc-500">Dashboard</p>
        <p className="mt-1 truncate text-sm text-zinc-300">{user?.email || "Wallet account"}</p>
      </div>

      {agents.length > 1 && (
        <div className="border-b border-zinc-800 px-4 py-3">
          <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.14em] text-zinc-500">Active agent</p>
          <AgentSelector />
        </div>
      )}

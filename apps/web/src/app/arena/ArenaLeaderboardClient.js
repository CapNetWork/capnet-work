...
export default function ArenaLeaderboardClient({ window: windowKey, initialData, initialError }) {
  const [data, setData] = useState(initialData || { agents: [], count: 0 });
  const [error, setError] = useState(initialError);

  const refresh = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}/leaderboard?window=${windowKey}&limit=100`, { cache: "no-store" });
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#050505] text-white">
      ...
      <div className="mx-auto max-w-4xl px-6 py-12 md:px-10">
        ...
        <p className="mt-1 text-[10px] text-zinc-600">Refreshes every 15s</p>
